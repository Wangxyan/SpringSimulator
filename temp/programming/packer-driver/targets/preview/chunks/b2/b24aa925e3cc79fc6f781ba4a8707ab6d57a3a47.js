      ...
      _export("Util", Util = class Util extends Component {
        ...
        */
        ...
        static dayRemainingTime() {
          ...
          var milliseconds = new Date(new Date().toLocaleDateString()).getTime() + 24 * 60 * 60 * 1000;
          var diff = milliseconds - curMilliseconds;
        ...
        */
        ...
        static radian_to_angle(radian) {
          ...
          // 180 / π * 弧度
          // 计算出角度
          var angle = 180 / Math.PI * radian; // 返回弧度
        ...
        static vector_to_angle(vector) {
          ...
          var dir = v2(vector.x, vector.y).normalize(); // 计算出目标角度的弧度
        ...
        static stringToColor(colorStr) {
          var r = this.hex2int(colorStr[0], 16) + this.hex2int(colorStr[1], 1);
          var g = this.hex2int(colorStr[2], 16) + this.hex2int(colorStr[3], 1);
          var b = this.hex2int(colorStr[4], 16) + this.hex2int(colorStr[5], 1);
          return new Color(r, g, b, 255);
        }

        static hex2int(hex, digit) {
          var code = hex.charCodeAt(0);
        ...
        static getCircleRandPoint(r, pos) {
          var x = pos.x + (Math.random() - 0.5) * 2 * r;
          var y = pos.y + (Math.random() - 0.5) * 2 * r;
          return new Vec2(x, y);
        }
        /**
        * 以某点为圆心，生成圆周上等分点的坐标
        ...
        */
        ...
        static getCirclePoints(r, pos, count, randomScope, dir) {
          ...
          var points = [];
          var radians = dir * (Math.PI / 180) * Math.round(360 / count);
        ...
        */
        ...
        static getCircleRandomPoints(r, pos, count, randomScope) {
          ...
          var offerR = Math.random() * Math.PI;

          for (var i = 0; i < count; i++) {
            var x = pos.x + r * Math.sin(radians * i + offerR);
            var y = pos.y + r * Math.cos(radians * i + offerR);
        ...
        static getCirclePos(r, pos, angle) {
          ...
          var x = pos.y + r * Math.cos(radians);
          var y = pos.x + r * Math.sin(radians);
        ...
        static getUpDownRandomPoints(r, pos, count, randomScope) {
          ...
          for (var i = 0; i < count; i++) {
            var dir = Math.floor(Math.random() * 100) <= 50 ? 1 : -1;
            var x = (Math.random() - 0.5) * 2 * randomScope;
            var y = pos.y + (r + Math.random() * 60) * dir;
        ...
        static getLeftRightRectRandomPoints1(r, pos, count, limtMoveWidth, limtMoveHeight, randomScope) {
          ...
          var radians = Math.PI / 180 * Math.round(360 / count);
          var offerR = Math.random() * Math.PI;
          var rec = new Rect(-limtMoveWidth / 2, -limtMoveHeight / 2, limtMoveWidth, limtMoveHeight);
          var index = 0;

          for (var i = 0; i < count; i++) {
            var x = pos.x + r * Math.sin(radians * i + offerR);
            var y = pos.y + r * Math.cos(radians * i + offerR); //判断当前坐标是否在方形内内

            var newPos = new Vec2(x + Math.random() * randomScope, y + Math.random() * randomScope);
            ...
            if (rec.contains(newPos)) {
              ...
              if (points.length) {
                ...
                var copyPos = this.clone(points[index]);
                var randPos = new Vec3(copyPos.x + (Math.random() - 0.5) * 2 * 20, copyPos.y + (Math.random() - 0.5) * 2 * 20, 0);
                points.unshift(randPos);
              } else {
                //方形内随机取随机取
                r = limtMoveHeight / 2 - randomScope;
                var newPosArr = this.getCircleRandomPoints(r, new Vec3(0, 0, 0), 10, 20);
        ...
        */
        ...
        static getBezierPoint(begin, ctrl1, ctrl2, end, t) {
          var x = begin.x * (1 - t) * (1 - t) * (1 - t) + 3 * ctrl1.x * t * (1 - t) * (1 - t) + 3 * ctrl2.x * t * t * (1 - t) + end.x * t * t * t;
          var y = begin.y * (1 - t) * (1 - t) * (1 - t) + 3 * ctrl1.y * t * (1 - t) * (1 - t) + 3 * ctrl2.y * t * t * (1 - t) + end.y * t * t * t;
          return v2(x, y);
        }
        /**
        *  二阶贝塞尔曲线 运动
        ...
        */
        ...
        static targetBezier(target, startPos, targetPos, time, px1, py1, px2, py2) {
          return new Promise((resolve, reject) => {
            ...
            var prePos = new Vec2(startPos.x, startPos.y);
            tween(tweenObj).to(time, {
              ...
              onUpdate: (tar, t) => {
                ...
                var posx = (targetPos.x - startPos.x) * x + startPos.x;
                var posy = (targetPos.y - startPos.y) * y + startPos.y;
                var curPos = new Vec2(posx, posy);
                target.setPosition(posx, posy);
                var vector = new Vec2();
                Vec2.subtract(vector, curPos, prePos);
                var r = vector.signAngle(v2(0, 1));
                var angle = misc.radiansToDegrees(r);
                target.angle = -angle;
                prePos = curPos.clone();
              }
            }).call(() => {
        ...
        static bezier_length(start_point, ctrl1, ctrl2, end_point, count) {
          if (count === void 0) {
            count = 20;
          }
        ...
        static getRectRandPos(rect) {
          var pos = new Vec3();
          var randX = Math.random() * rect.width;
          var randY = Math.random() * rect.height;
          pos.x = rect.x + randX;
          pos.y = rect.y + randY;
          return pos;
        }
        ...
        static adapterScale() {
          ...
          var scale = visibleSize.width / designSize.width;
          return scale;
        }
