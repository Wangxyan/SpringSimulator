    ...
    execute: function () {
      ...
      _export("MonsterManager", MonsterManager = (_dec = ccclass('MonsterManager'), _dec2 = property(Node), _dec(_class = (_class2 = class MonsterManager extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        setBrushMonsterEvent() {
          this._curTotalNum = 0;
          this._curKillNum = 0;
          var eType = this._curChapterEvent.eType;

          var eventArr = this._curChapterEvent.Event.split(','); // if (eType == BrushMonsterType.BOSS) {
          //     //boss事件(随机boss事件)
          ...
          for (var i = 0; i < eventArr.length; i++) {
            var eventStrArr = eventArr[i].split('|');
            var monsterId = Number(eventStrArr[0]);
            var totalNum = Number(eventStrArr[1]);
            var startTime = Number(eventStrArr[2]);
            var endTime = Number(eventStrArr[3]);
            var diffTime = endTime - startTime;
            var secNum = Math.floor(totalNum / diffTime);
            var diffNum = totalNum - diffTime * secNum;
            var monsterEventObj = new (_crd && MonsterEventObj === void 0 ? (_reportPossibleCrUseOfMonsterEventObj({
              error: Error()
        ...
        getBossEventByWeight(eventArr) {
          ...
          for (var i = 0; i < eventArr.length; i++) {
            var element = eventArr[i];
            var eventStrArr = element.split('|');
            var weight = Number(eventStrArr[4]);
        ...
        onCreateMonster(createrNum) {
          ...
          var _loop = function _loop() {
            if (_this._queeuMonsterArr.length) {
              var monsterObj = _this._queeuMonsterArr.shift();

              var monsterId = monsterObj.id;
              var monsterItem = null;
              var pos = new Vec3();
              var monsterCfg = (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
                error: Error()
              }), MonsterData) : MonsterData).instance.getMonsterCfgById(monsterId);

              if (_this._monsterPrefabMap.has(monsterId)) {
                ...
                monsterItem = (_crd && PoolManager === void 0 ? (_reportPossibleCrUseOfPoolManager({
                  error: Error()
                }), PoolManager) : PoolManager).instance.getNodeItemById(_this._monsterPrefabMap.get(monsterId), monsterId.toString());

                _this.monsterParent.addChild(monsterItem);
                ...
                (_crd && ResourcesUtil === void 0 ? (_reportPossibleCrUseOfResourcesUtil({
                  ...
                  //加载资源
                  _this.loadMonsterRes(monsterObj, monsterCfg, monsterPrefab);
                });
              }
            }
          };

          for (var i = 0; i < createrNum; i++) {
            _loop();
        ...
        loadMonsterRes(monsterObj, monsterCfg, monsterPrefab) {
          ...
          var iconPath = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          ...
          (_crd && ResourcesUtil === void 0 ? (_reportPossibleCrUseOfResourcesUtil({
            ...
            var iconSprite = monsterItem.getChildByName('icon').getComponent(Sprite);
            iconSprite.spriteFrame = data;
          }); //加载动画资源

          var animPath = (_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          ...
          (_crd && ResourcesUtil === void 0 ? (_reportPossibleCrUseOfResourcesUtil({
            ...
            if (asert.length) {
              var pos = monsterCfg.mType != (_crd && MonsterType === void 0 ? (_reportPossibleCrUseOfMonsterType({
                error: Error()
              }), MonsterType) : MonsterType).BOSS ? (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
                error: Error()
              }), MonsterData) : MonsterData).instance.getScreenUpRandPos() : (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
              ...
              var animationCom = monsterItem.getChildByName('icon').getComponent(AnimationComponent);
              this.monsterParent.addChild(monsterItem);
              monsterItem.setPosition(pos);

              for (var i = 0; i < asert.length; i++) {
                var clip = asert[i];
        ...
        onMonsterDie(pos) {
          ...
          if (!this.monsterParent.children.length) {
            ...
            if (this._curChapterEvent.Wave >= (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
              ...
              var wave = this._curChapterEvent.Wave + 1; //检测是否有宝箱
        ...
        triggleBrushMonsterEvent(dt) {
          ...
          if (this._schduleTime <= 0) {
            for (var i = 0, len = this._brushMonsterEventArr.length; i < len; i++) {
              var event = this._brushMonsterEventArr[i];

              if (event.curNum < event.totalNum) {
                if (this._gameTime >= event.startTime) {
                  this._schduleTime = 1; //加入队列怪物

                  var num = event.secNum;
                  ...
                  for (var n = 0; n < num; n++) {
                    var monsterObj = new (_crd && MonsterObj === void 0 ? (_reportPossibleCrUseOfMonsterObj({
                      error: Error()
        ...
        setMonsterSort() {
          var monsterItems = this.monsterParent.children;
          monsterItems.sort((x, y) => {
            return Math.round(y.position.y) - Math.round(x.position.y);
          });

          for (var i = 0; i < monsterItems.length; i++) {
        ...
        childUpdate(dt) {
          if ((_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            ...
            if (this._queeuMonsterArr.length) {
              var createNum = this._queeuMonsterArr.length > 60 ? Math.ceil(this._queeuMonsterArr.length / 60) : 1;
