    ...
    execute: function () {
      ...
      _export("GridData", GridData = class GridData {
        ...
        getWeaponCharmByKey(key) {
          var weaponObj = this._gridItemData.get(key);

          var weaponCfg = (_crd && WeaponData === void 0 ? (_reportPossibleCrUseOfWeaponData({
            error: Error()
          }), WeaponData) : WeaponData).instance.getWeaponCfgById(weaponObj.wid);
          var values = weaponCfg.charm;
          return values;
        }
        /*获取武器的知识加成*/
        ...
        getWeaponKnowledgeByKey(key) {
          ...
          var values = weaponCfg.knowledge;
          return values;
        }
        /*获取武器的才艺加成*/
