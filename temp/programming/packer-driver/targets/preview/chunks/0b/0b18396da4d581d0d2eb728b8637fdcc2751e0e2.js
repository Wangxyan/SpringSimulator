  ...
  return {
    ...
    execute: function () {
      ...
      _export("ChapterMapData", ChapterMapData = class ChapterMapData {
        ...
        setChapterMapData() {
          var chapterId = (_crd && PlayerData === void 0 ? (_reportPossibleCrUseOfPlayerData({
            error: Error()
        ...
        setChapterRewardData(chapterMapRewardData, rewardStr) {
          ...
          var chapterDropRewardObj = null;
          var itemId = Number(rewardArr[0]);
          var num = Number(rewardArr[1]);
        ...
        getChapterRewardData(isWin) {
          //TODO 掉落奖励
          var chapterMapRewardData = new Map();

          if (this._chapterMapData.StarReward) {
            //开始奖励
            var StarRewardArr = this._chapterMapData.StarReward.split(',');

            for (var i = 0; i < StarRewardArr.length; i++) {
              var rewardStr = StarRewardArr[i];
          ...
          if (isWin) {
            ...
            if (this._chapterMapData.WinReward) {
              var WinRewardArr = this._chapterMapData.WinReward.split(',');

              for (var _i = 0; _i < WinRewardArr.length; _i++) {
                var _rewardStr = WinRewardArr[_i];
                this.setChapterRewardData(chapterMapRewardData, _rewardStr);
              }
            } //首次通关奖励
