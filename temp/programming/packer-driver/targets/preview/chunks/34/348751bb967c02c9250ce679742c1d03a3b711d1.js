    ...
    execute: function () {
      ...
      _export("HeroBase", HeroBase = (_dec = ccclass('HeroBase'), _dec2 = property(sp.Skeleton), _dec3 = property(Node), _dec4 = property(Prefab), _dec(_class = (_class2 = class HeroBase extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        getFirePos() {
          ...
          var firePos = new Vec3(pos.x + this._firePos.x, pos.y + this._firePos.y, 0);
          return firePos;
        }
        /**
        * 更新间隔伤害时间
        ...
        */
        ...
        releaseSkill() {
          if (this.releaseSkillStatus) {
            var releaseSkillObj = (_crd && HeroData === void 0 ? (_reportPossibleCrUseOfHeroData({
              error: Error()
            ...
            if (releaseSkillObj) {
              ...
              this.scheduleOnce(() => {
                var skillBulletObj = new (_crd && SkillBulletObj === void 0 ? (_reportPossibleCrUseOfSkillBulletObj({
                  error: Error()
                ...
                var interval = 0.1;
                var releaseTimes = skillBulletObj.skillCfg.Salvo;
                var targetCount = 1; //释放次数

                for (var i = 0; i < releaseTimes; i++) {
                  var targetPosArr = (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
                    error: Error()
                  }), MonsterData) : MonsterData).instance.getNearMonster(skillBulletObj.skillKey);

                  if (targetPosArr.length) {
                    //子弹个数
                    var bulletCount = skillBulletObj.skillCfg.BulletCount;

                    for (var j = 0; j < bulletCount; j++) {
                      //目标数量
                      for (var n = 0; n < targetCount; n++) {
                        var intervalAngle = skillBulletObj.skillCfg.IntervalAngle;
                        var angle = (_crd && SkillData === void 0 ? (_reportPossibleCrUseOfSkillData({
                          error: Error()
