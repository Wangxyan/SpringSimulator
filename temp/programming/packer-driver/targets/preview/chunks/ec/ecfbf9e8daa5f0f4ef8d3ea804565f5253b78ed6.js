    ...
    execute: function () {
      ...
      _export("MonsterData", MonsterData = class MonsterData {
        ...
        addSkillRangeMonster(skillPos, range, monsterComMap) {
          var monsterBaseArr = Array.from(this._monsterNodeMap.values());

          for (var i = 0; i < monsterBaseArr.length; i++) {
            var element = monsterBaseArr[i];
            var monsterPos = element.curPos;
            var monsteKey = element.monsterKey;
            var len = Vec3.distance(skillPos, monsterPos);
        ...
        getNearMonster(skillKey) {
          ...
          if (this._nearMonsterData.has(skillKey)) {
            var monsterBaseArr = Array.from(this._nearMonsterData.get(skillKey).values());
        ...
        checkNearMonster(skillKey) {
          if (this._nearMonsterData.has(skillKey)) {
            var monsterComMap = this._nearMonsterData.get(skillKey);
        ...
        getAppointMonsterPoint(Offset) {
          if (Offset === void 0) {
            Offset = new Vec2(0, 100);
          }

          var len = this._brushMonsterPoint.size;
          var pid = Math.floor(Math.random() * len);

          var rect = this._brushMonsterPoint.get(pid); //获取区域内随机坐标点


          var pos = (_crd && Util === void 0 ? (_reportPossibleCrUseOfUtil({
            error: Error()
        ...
        getBossBrushPoint(Offset) {
          if (Offset === void 0) {
            Offset = new Vec2(100, 100);
          }

          var x = 0;
          var y = (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
            error: Error()
        ...
        getScreenUpRandPos(Offset) {
          if (Offset === void 0) {
            Offset = new Vec2(60, 50);
          }

          var visibleSize = view.getVisibleSize();
          var width = visibleSize.width - Offset.x;
          var pos = new Vec3();
