    ...
    execute: function () {
      ...
      _export("MonsterBase", MonsterBase = (_dec = ccclass('MonsterBase'), _dec2 = property(AnimationComponent), _dec(_class = (_class2 = class MonsterBase extends (_crd && BaseComponent === void 0 ? (_reportPossibleCrUseOfBaseComponent({
        ...
        setMonsterBorn() {
          ...
          this.isMove = true;
          this.atkTime = 0;
          var comObj = this.monsterAnim.getComponent(Sprite);
          comObj.color = new Color(255, 255, 255);
          this.setMonsterSize();
          var monterName = this.monsterCfg.mType == (_crd && MonsterType === void 0 ? (_reportPossibleCrUseOfMonsterType({
            error: Error()
        ...
        */
        ...
        getColliderPos() {
          var pos = this.curPos.clone();
          var offset = this.boundBox.offset;
          pos.x += offset.x;
          pos.y += offset.y;
          return pos;
        }
        ...
        setMonsterBoundBox() {
          ...
          var offsetArr = this.monsterCfg.ColliderOffset ? this.monsterCfg.ColliderOffset.split('|') : ['0', '0'];
        ...
        getBoundBoxCircle() {
          var circleR = this.boundBox instanceof CircleCollider2D ? this.boundBox.radius : 20;
          return circleR;
        }
        /**
         * 怪物移动
        ...
        monsterMoveFrame(dt) {
          if (!this.isRepulse) {
            if (this.isMove) {
              ...
              var dis = dir.multiply(v3(this.moveSpeed * dt, this.moveSpeed * dt, 0));
              var temPos = nodePos.add(dis);
        ...
        onAttacked(attack, type, skillBulletObj) {
          if (type === void 0) {
            type = 0;
          }

          if (skillBulletObj === void 0) {
            skillBulletObj = null;
          }

          if (attack > 0 && !this.isDie) {
            var attacked = Math.ceil(attack);
        ...
        playHit() {
          ...
          var tmpColor = color(255, 255, 255, 50);
        ...
        setMonsterHurtMoveEffect(move, intensity_up, dir, pos) {
          if (move.length) {
            var moveArr = move[0];
            var type = Number(moveArr[0]);
            var speed = Number(moveArr[1]) + intensity_up;
            var time = Number(moveArr[2]);
          ...
          if (this.isRepulse) {
            if (this._moveEffect) {
              ...
              if (this._moveEffect.time <= 0) {
                ...
                ;
                var dir = this._moveEffect.dir;
                var speed = this._moveEffect.speed;
                var dis = dir.multiply(v3(speed * dt, speed * dt, 0));
                var temPos = nodePos.add(dis); //是否出边界
                ...
                if (Math.abs(temPos.x) > (_crd && GameDefine === void 0 ? (_reportPossibleCrUseOfGameDefine({
                  ...
                  var _dir = new Vec3(0, 1, 0);

                  dis = _dir.multiply(v3(speed * dt, speed * dt, 0));
