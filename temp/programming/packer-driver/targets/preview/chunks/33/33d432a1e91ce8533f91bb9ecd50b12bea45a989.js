      ...
      _export("SkillData", SkillData = class SkillData {
        ...
        */
        ...
        upgradeSkillObjAtt(skillUpgradeCfg) {
          //aim_skill   0:针对所有
          var aim_skill = skillUpgradeCfg.aim_skill;

          if (aim_skill) {
            for (var i = 0; i < aim_skill.length; i++) {
              var skillId = aim_skill[i];
              var skillUpgradeObj = this._battleSkillUpgradeData.has(skillId) ? this._battleSkillUpgradeData.get(skillId) : new SkillUpgradeObj();
        ...
        getUpgradeSkillBySkillId(skillCfg) {
          var skillCfgTem = (_crd && Util === void 0 ? (_reportPossibleCrUseOfUtil({
            error: Error()
          }), Util) : Util).clone(skillCfg);

          if (this._battleSkillUpgradeData.has(skillCfgTem.Id)) {
            var upgradeObj = this._battleSkillUpgradeData.get(skillCfgTem.Id);

            for (var key in upgradeObj) {
              if (Object.prototype.hasOwnProperty.call(upgradeObj, key)) {
                var element = upgradeObj[key];
        ...
        */
        ...
        getSkillBulletCount(targetId) {
          if (targetId) {
            var targetCfg = (_crd && CfgMgr === void 0 ? (_reportPossibleCrUseOfCfgMgr({
              error: Error()
            ...
            if (targetCfg.random) {
              var randomArr = targetCfg.random.split(':');
        ...
        getSkillAtkTargetPos(targetId, curPos, skillKey) {
          ...
          if (targetId) {
            ...
            var targetType = targetCfg.type;
            var random = targetCfg.random;
            var randomArr = random.split(':');
            var poolCount = Number(randomArr[0]);
            var targetCount = Number(randomArr[1]);
        ...
        getHeroNearTargetPos(status, poolCount, targetCount, heroKey, curPos) {
          var posArr = [];
          var monsterNodeMap = (_crd && MonsterData === void 0 ? (_reportPossibleCrUseOfMonsterData({
            error: Error()
          ...
          if (monsterNodeMap.size) {
            ...
            if (checkArr.length) {
              for (var i = 0; i < targetCount; i++) {
                var index = i < checkArr.length ? Math.floor(Math.random() * checkArr.length) : 0;
                var e = checkArr[index];
        ...
        getScreenTargetPos(poolCount, targetCount, heroKey) {
          ...
          if (monsterNodeMap.size) {
            var data = Array.from(monsterNodeMap.values());
        ...
        getSelfTargetPos(status, poolCount, targetCount, heroKey, curPos) {
          ...
          if (monsterNodeMap.size) {
            monsterNodeMap.forEach(item => {
              //垂直距离
              var len = Vec3.distance(item.getColliderPos(), curPos);
