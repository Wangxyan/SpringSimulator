  ...
  return {
    ...
    execute: function () {
      ...
      _export("Configuration", Configuration = class Configuration {
        ...
        init() {
          let localdata = sys.localStorage.getItem((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
        ...
        clearConfigDataToVersion() {
          let loacalCacheVersionConfig = this.getConfigData((_crd && Constants === void 0 ? (_reportPossibleCrUseOfConstants({
            error: Error()
          }), Constants) : Constants).cacheVersion);

          if (loacalCacheVersionConfig) {
            let loacalCacheVersionConfigArr = JSON.parse(loacalCacheVersionConfig); //比较本地版本号

            let copyVerson = (_crd && Util === void 0 ? (_reportPossibleCrUseOfUtil({
              error: Error()
